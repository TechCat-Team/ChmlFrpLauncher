...
pub async fn ping_host(host: String) -> Result<PingResult, String> {
    info!("Starting ping test for host: {}", host);
    ...
    tokio::task::spawn_blocking(move || {
        ...
        debug!("Executing ping command for: {}", host);
        ...
        match output {
            Ok(output) => {
                ...
                info!("Ping to {} - Exit code: {:?}", host, output.status.code());
                debug!("Ping stdout for {}: {}", host, stdout);
                if !stderr.is_empty() {
                    warn!("Ping stderr for {}: {}", host, stderr);
                }
                ...
                if output.status.success() {
                    ...
                    if let Some(latency) = latency_str.or(latency_bytes) {
                        info!("Ping to {} successful: {}ms", host, latency);
                        Ok(PingResult {
                            success: true,
                        ...
                        warn!("Ping to {} - Parse failed: {}", host, debug_info);
                        Ok(PingResult {
                            success: false,
                            latency: None,
                    ...
                    let error_summary = error_msg.lines().next().unwrap_or("Unknown error");
                    error!("Ping to {} failed: {}", host, error_summary);
                    
                    Ok(PingResult {
                        success: false,
                        latency: None,
                        error: Some(format!("Ping failed: {}", error_summary)),
                        raw_output: Some(stdout.to_string()),
                    })
                }
            }
            Err(e) => {
                error!("Failed to execute ping for {}: {}", host, e);
                Ok(PingResult {
                    success: false,
                    latency: None,
    ...
    .await
    .map_err(|e| {
        error!("Task join error for ping: {}", e);
